...
function MM_openBrWindow(theURL, winName, features) {
    var path = "";
    for (var i = 3; i < document.URL.split('/').length - 1; i++) {
        path += "/" + document.URL.split('/')[i];
    }

    if (theURL.charAt(0) != '/') {
        theURL = path + '/' + theURL;
    }
    viewer = document.open(theURL, winName, features); viewer.focus();
}
...
var gblSeed;//see getSuspendData
var gblSeedStep = 0;
/*function initSeed() {
...
function genRandomWithSeed() {
    if (gblSeed === "undefined") gblSeed = Math.random();
...
function getPreviousScreen() {
	...
	/*  FOLLOWING ADDED BY DAN HERR - Force Final Assessment Intro Screen to go back to menu - AUGUST 2016 */
	if (currScreen.id == "64696") {
		//Lesson 6 Summary Screen = screen64696.html
		for (var j = 0; j < gblScreens.length; j++) {
			if (gblScreens[j].id === "76439") {
				//Main Menu = screen76439.html
				return gblScreens[j];
    ...
    for (var i = startIndex; i >= 0; i--) {
        ...
        if (goToPageNum > 0 && currScreenFound == true && gblScreens[i].pageNumber == goToPageNum && currScreen.parentModule.rootModuleIsResource == gblScreens[i].parentModule.rootModuleIsResource) {
            // <first condition> : don't go before the first screen, there is only something there if there are multiple sco's in the 'course' in the LMS 
            if (!(gblScreens[i].parentModule.rootModuleIsResource) || (gblScreens[i].parentModule.rootModuleIsResource && gblScreens[i].parentModule.rootModule.id == currScreen.parentModule.rootModule.id)) {
...
function toggleSound() {
    ...
    var navDoc = navFrame.contentWindow.document;

    var frameScreen = navDoc.getElementById("frameScreen");
    var frameDoc = frameScreen.contentWindow.document;

    var narrAudio = frameDoc.getElementById("jp_audio_0");
    if (narrAudio != null) {
        if (narrAudio.paused) {
            narrAudio.play();
        } else {
...
function LMSInitialize() {
    ...
    if (isSCORM_1_2) {
		/*  FOLLOWING CHANGED BY DAN HERR - gblSCORMAPI returns an object not a string.  - AUGUST 2016 */
		//return gblSCORMAPI.LMSInitialize("");
		return gblSCORMAPI.LMSInitialize("").toString();
    }
    else
...
function LMSFinish() {
    ...
    if (isSCORM_1_2)
		...
		return gblSCORMAPI.LMSFinish("").toString();
    else
...
function LMSGetValue(parameter) {
    if (isSCORM_1_2)
		...
		return gblSCORMAPI.LMSGetValue(parameter).toString();
    else
...
function LMSSetValue(parameter, value) {
    if (isSCORM_1_2)
		...
		return gblSCORMAPI.LMSSetValue(parameter, value).toString();
    else
...
function LMSCommit() {
    ...
    if (isSCORM_1_2)
		...
		return gblSCORMAPI.LMSCommit("").toString();
    else
...
function getBookmark() {
	...
	/*  FOLLOWING ADDED BY DAN HERR - AUGUST 2016 */
	/* Four Screen Name/Numbers changed since last version, so need to replace old number with new one in lesson_location - Dan Herr */
	if (intScreenID == "64450") intScreenID = "76439";
	if (intScreenID == "64467") intScreenID = "76438";
	if (intScreenID == "64541") intScreenID = "76441";
	if (intScreenID == "64736") intScreenID = "76443";
	/* END ADDITION */
...
function setLogout() {
    if (gblCourse.completed) {
        ...
        if (top.window == window) {
            ...
            if (isSCORM_1_2) {
				/*  FOLLOWING REMOVED BY DAN HERR - adl.nav.request NOT IN SCORM 1.2 DATA MODEL.  - AUGUST 2016 */
    ...
    if (intArrLen != null && intArrLen > 0) {
        ...
        for (i = 0; i < intArrLen; i++) {
            ...
            if (id != null && id != '') {
				...
				//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - 'incorrect' IS NOT A VALID cmi.interactions.n.result VALUE IN SCORM 1.2 DATA MODEL
				if (isSCORM_1_2 && (result == 'wrong')) result = 'incorrect';

				
				//FOLLOWING CHANGED BY DAN HERR, AUGUST 2016 - cmi.interactions.n.description NOT IN SCORM 1.2 DATA MODEL
				...
				//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.correct_responses.n.pattern IN SCORM 1.2 DATA MODEL
				if (isSCORM_1_2) correct_response = correct_response.split(",").join("[,]");
				
				//FOLLOWING CHANGED BY DAN HERR, AUGUST 2016 - cmi.interactions.n.learner_response NOT IN SCORM 1.2 DATA MODEL
				//var learner_response = LMSGetValue('cmi.interactions.' + i + '.learner_response');
				if (isSCORM_1_2) {
					var learner_response = LMSGetValue('cmi.interactions.' + i + '.student_response');
					//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.student_response IN SCORM 1.2 DATA MODEL
					learner_response = learner_response.split(",").join("[,]");
				} else {
					var learner_response = LMSGetValue('cmi.interactions.' + i + '.learner_response');
				}
    ...
    if (true) {
        ...
        if (gblSCORMAPI != null) {
            for (var i = 0; i <= arrInteractions.length - 1; i++) {//null out the current session interactions
				...
				//FOLLOWING CHANGED BY DAN HERR, AUGUST 2016 - 'incorrect' AND 'partial' ARE NOT A VALID cmi.interactions.n.result VALUE IN SCORM 1.2 DATA MODEL
				//LMSSetValue('cmi.interactions.' + intSCORMindex + '.result', arrInteractions[intSCORMindex].result);
				if (isSCORM_1_2 && ((arrInteractions[intSCORMindex].result == 'incorrect') || (arrInteractions[intSCORMindex].result == 'partial'))) {
					LMSSetValue('cmi.interactions.' + intSCORMindex + '.result', 'wrong');
				} else {
					LMSSetValue('cmi.interactions.' + intSCORMindex + '.result', arrInteractions[intSCORMindex].result);
				}
				...
				if (!isSCORM_1_2) LMSSetValue('cmi.interactions.' + intSCORMindex + '.description', arrInteractions[intSCORMindex].description);


                //assume interaction is a mult-choice for now (this will change)


				//FOLLOWING CHANGED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.correct_responses.n.pattern IN SCORM 1.2 DATA MODEL
				...
				if (correct_response != null) {
					var v = arrInteractions[intSCORMindex].correct_response;
					if (isSCORM_1_2) v = v.split("[").join("").split("]").join("");
					LMSSetValue('cmi.interactions.' + intSCORMindex + '.correct_responses.0.pattern', v);
				}
				...
				if (learner_response != null)
					...
					if (isSCORM_1_2) {
						//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.student_response IN SCORM 1.2 DATA MODEL
						var v = arrInteractions[intSCORMindex].learner_response;
						v = v.split("[").join("").split("]").join("");
						if (v != "null") LMSSetValue('cmi.interactions.' + intSCORMindex + '.student_response', v);
					} else {
						LMSSetValue('cmi.interactions.' + intSCORMindex + '.learner_response', arrInteractions[intSCORMindex].learner_response);
					}
			}
        ...
        if (gblSCORMAPI != null) {
            for (var i = 0; i <= arrPreviousSessionInteractions.length - 1; i++) {//null out previous session interactions
				...
				if (isSCORM_1_2 && ((arrPreviousSessionInteractions[i].result == 'incorrect') || (arrPreviousSessionInteractions[i].result == 'partial'))) {
				...
				if (!isSCORM_1_2) LMSSetValue('cmi.interactions.' + i + '.description', arrPreviousSessionInteractions[i].description);
				...
				if (correct_response != null) {
					var v = arrPreviousSessionInteractions[i].correct_response;
					if (isSCORM_1_2) v = v.split("[").join("").split("]").join("");
					LMSSetValue('cmi.interactions.' + i + '.correct_responses.0.pattern', v);
				}
				...
				if (learner_response != null)
					...
					if (isSCORM_1_2) {
						//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.student_response IN SCORM 1.2 DATA MODEL
						var v = arrPreviousSessionInteractions[i].learner_response;
						v = v.split("[").join("").split("]").join("");
						if (v != "null") LMSSetValue('cmi.interactions.' + i + '.student_response', v);
					} else {
						LMSSetValue('cmi.interactions.' + i + '.learner_response', arrPreviousSessionInteractions[i].learner_response);
					}
        ...
        if (ignoreCorrect != true) {
            ...
            if (gblSCORMAPI != null) {
                for (var i = 0; i <= arrInteractions.length - 1; i++) {
					...
					if (isSCORM_1_2 && ((arrInteractions[i].result == 'incorrect') || (arrInteractions[i].result == 'partial'))) {
						LMSSetValue('cmi.interactions.' + intSCORMindex + '.result', 'wrong');
					} else {
						LMSSetValue('cmi.interactions.' + intSCORMindex + '.result', arrInteractions[i].result);
					}
					...
					if (!isSCORM_1_2) LMSSetValue('cmi.interactions.' + intSCORMindex + '.description', arrInteractions[i].description);
					...
					if (correct_response != null) {
						var v = arrInteractions[i].correct_response;
						if (isSCORM_1_2) v = v.split("[").join("").split("]").join("");
						LMSSetValue('cmi.interactions.' + intSCORMindex + '.correct_responses.0.pattern', v);
					}
					...
					if (learner_response != null)
						...
						if (isSCORM_1_2) {
							//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - SQUARE BRACKETS ARE NOT VALID IN cmi.interactions.n.student_response IN SCORM 1.2 DATA MODEL
							var v = arrInteractions[i].learner_response;
							v = v.split("[").join("").split("]").join("");
							if (v != "null") LMSSetValue('cmi.interactions.' + intSCORMindex + '.student_response', v);
						} else {
							LMSSetValue('cmi.interactions.' + intSCORMindex + '.learner_response', arrInteractions[i].learner_response);
...
function setCurrScreenInteractionDescription(description) {
    ...
    if (intArrIndex >= 0) {
        ...
        if (gblSCORMAPI != null) {
			...
			if (!isSCORM_1_2) LMSSetValue('cmi.interactions.' + intSCORMindex + '.description', description);
...
function setInteractionStatus(id, result, description, correct_response, learner_response, type) {
  	...
  	//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - DO NOT SAVE ANY cmi.interactions DATA
 	if (gblSCORMAPI != null) saveProgress(true);
 	return;
 	...
 	if (gblSCORMAPI != null) {
 		...
 		//FOLLOWING ADDED BY DAN HERR, AUGUST 2016 - 'incorrect' AND 'partial' ARE NOT A VALID cmi.interactions.n.result VALUE IN SCORM 1.2 DATA MODEL
 		if (isSCORM_1_2 && ((result == 'incorrect') || (result == 'partial'))) result = 'wrong';
...
function getSuspendData() {
	...
	/*  FOLLOWING ADDED BY DAN HERR - AUGUST 2016 */
	/* Four Screen Name/Numbers changed since last version, so need to replace old number with new one in suspend_data - Dan Herr */
	tmp = tmp.split("64450").join("76439");
	tmp = tmp.split("64467").join("76438");
	tmp = tmp.split("64541").join("76441");
	tmp = tmp.split("64736").join("76443");
	/* END ADDITION */
...
function saveProgress(blnCommit) {
    if (gblSCORMAPI != null) {
        gblCourse.completed = evaluateCourseCompletion();
 		...
 		/* FOLLOWING CHANGED BY DAN HERR, AUGUST 2016 - Final Assessment will set completion
 		if (gblCourse.completed)
 			setCompleted();
 		else
 			setIncomplete();
